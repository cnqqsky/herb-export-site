 ...
 * 模板与数据已在 build-templates.mjs 阶段预编译到 functions/templates.js。
 */
import ejs from 'ejs';
...
function baseFor(env, lang, t, company, categories, langUrl) {
  ...
  base.renderCard = (p) => ejs.render(templates.productCardTemplate, Object.assign({}, base, { p }));
  return base;
}
...
async function layout404(env, lang, url) {
  ...
  const html = ejs.render(templates['404'], base);
...
async function renderHome(env, base, lang, url) {
  ...
  const html = ejs.render(templates.home, Object.assign({}, base, {
...
async function renderProducts(env, base, lang, url) {
  ...
  const html = ejs.render(templates.products, Object.assign({}, base, {
...
async function renderProduct(env, base, lang, slug, url) {
  ...
  const html = ejs.render(templates.product, Object.assign({}, base, {
    p, related,
...
async function renderAbout(env, base, lang, url) {
  ...
  const html = ejs.render(templates.about, Object.assign({}, base, {
    certs,
...
async function renderCerts(env, base, lang, url) {
  const certs = await db.all(env, 'SELECT * FROM certificates ORDER BY sort_order, id');
  const html = ejs.render(templates.certificates, Object.assign({}, base, {
    certs,
...
async function renderContact(env, base, lang, url) {
  ...
  const html = ejs.render(templates.contact, Object.assign({}, base, {
    product,
